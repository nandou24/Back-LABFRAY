...
const crearRefMedico = async (req, res = response) => {
    try {
        ...
        const ultimoRefMedico = await RefMedico.findOne({}, { codRefMedico: 1 }).sort({ codRefMedico: -1 }).lean();
        let correlativo = 1;
        if (ultimoRefMedico) {
            const ultimoCorrelativo = parseInt(ultimoRefMedico.codRefMedico.substring(2), 10);
...
const actualizarRefMedico = async (req, res = response) => {
    ...
    if (datosActualizados.profesionesRefMedico) delete datosActualizados.profesionesRefMedico._id;
    if (datosActualizados.especialidadesRefMedico) delete datosActualizados.especialidadesRefMedico._id;
    if (datosActualizados.profesionSolicitante) delete datosActualizados.profesionSolicitante._id;
    try {
        const refMedico = await RefMedico.findOneAndUpdate({ codRefMedico: codigo }, datosActualizados);
        if (!refMedico) {
